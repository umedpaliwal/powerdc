...
export default function DashboardFilters({ filters, setFilters, plantData, onFilterChange, onReset }: DashboardFiltersProps) {
  ...
  const getUniqueValues = (field: string) => {
    ...
    if (filters.rto_name && field !== 'rto_name') {
      filteredPlants = filteredPlants.filter(plant => 
        plant.rto_name === filters.rto_name
      );
    }

    if (filters.state && field !== 'rto_name' && field !== 'state') {
      filteredPlants = filteredPlants.filter(plant => 
        plant.state === filters.state
      );
    }

    if (filters.parent_company_name && 
        field !== 'rto_name' && 
        field !== 'state' && 
        field !== 'parent_company_name') {
      filteredPlants = filteredPlants.filter(plant => 
        plant.parent_company_name === filters.parent_company_name
      );
    }
